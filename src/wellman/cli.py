...
def cmd_adopt(args: argparse.Namespace) -> int:
    ...
    if adopts_docs:
        try:
            # Resolve identity before creating .governance so a failed
            # bootstrap cannot leave a partially adopted repository behind.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_adopt.add_argument(
        "--repository",
        help="Canonical GitHub owner/name when origin is unavailable",
    )
